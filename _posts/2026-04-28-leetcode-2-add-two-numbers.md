...
## 解題心得：直式加法的鏈表版
這題考的是對鏈表指針的操作，以及對「進位 (Carry)」的處理。

**核心邏輯：**
1. **同步遍歷**：同時走這兩條鏈表。
2. **個位相加**：每一位的值 = `(node1.val + node2.val + carry) % 10`。
3. **處理進位**：下一位的 `carry` = `(node1.val + node2.val + carry) // 10`。
4. **補位**：如果一條鏈表比較短，就把它當作 0 處理。
5. **最後的進位**：全部加完後，如果還有剩餘的 `carry`，別忘了在最後補一個節點。

這就像是在小學寫直式加法。我們從個位開始加，滿十進一，唯一不同的是，我們現在是在鏈表上一個節點一個節點地做。
